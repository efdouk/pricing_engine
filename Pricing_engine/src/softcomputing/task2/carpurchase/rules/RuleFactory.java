...
public class RuleFactory {
	...
	private static Rule getRule(String attr) {
		...
		if(attr.equals("-")) return new ZeroRule();
		else if(attr.substring(0, 1).equals("/")) return new ReverseRule(Double.parseDouble(attr.substring(1)));
		else return new ProportionalRule(Double.parseDouble(attr));
	}
